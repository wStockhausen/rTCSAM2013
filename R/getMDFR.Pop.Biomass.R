#'
#'@title Get population biomass time series from model results from TCSAM2013 model runs as a dataframe
#'
#'@description Function to get population biomass time series from model results from TCSAM2013 model runs as a dataframe.
#'
#'@param obj - single tcsam2013.rep object, tcsam2013.resLst object, or named list of the latter
#'@param cast - casting formula (e.g., "x+m") for excluding x,m,s,z factor levels from a sum of B_yxmsz across the unspecified factors
...
getMDFR.Pop.Biomass<-function(obj,cast="y+x",verbose=FALSE){
    ...
    if (verbose) cat("--starting rTCSAM2013::getMDFR.Pop.Biomass().\n");
    ...
    #----------------------------------
    # predicted biomass from population
    #----------------------------------
    rws<-rbind(data.frame(x='female',m='immature',s='new shell',var="pop.mod.BatZ.INF",stringsAsFactors=FALSE),
                     list(x='female',m='immature',s='old shell',var="pop.mod.BatZ.IOF"),
                     list(x='female',m=  'mature',s='new shell',var="pop.mod.BatZ.MNF"),
                     list(x='female',m=  'mature',s='old shell',var="pop.mod.BatZ.MOF"),
                     list(x=  'male',m='immature',s='new shell',var="pop.mod.BatZ.INM"),
                     list(x=  'male',m='immature',s='old shell',var="pop.mod.BatZ.IOM"),
                     list(x=  'male',m=  'mature',s='new shell',var="pop.mod.BatZ.MNM"),
                     list(x=  'male',m=  'mature',s='old shell',var="pop.mod.BatZ.MOM"));
    dfr<-NULL;
    for (case in cases){
        if (verbose) {
            cat("Processing",case,"\n");
    ...
    if (verbose) cat("--finished rTCSAM2013::getMDFR.Pop.Biomass(). \n");
    return(mdfr);
}
